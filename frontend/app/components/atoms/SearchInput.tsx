...
export default function SearchInput(props: React.HTMLAttributes<HTMLDivElement>) {
  const { className, ...propsWithoutClassName } = props?.className ? { className: props.className, ...props } : { ...props, className: "" };
  return (
    <div className={`relative w-full max-w-md ${className}`} {...propsWithoutClassName}>
      <input
        type="text"
        placeholder="検索"
